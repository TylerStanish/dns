...
fn validate_blocklist_entry(s: &str) -> Result<(String, bool), String> {
    ...
    if pos != s.rfind("*.") {
        ...
        return Err(format!(
            "Blocklist entry contained more than one '*.': {}",
            s
        ));
    }
    // strip wildcard
...
pub fn load_blocklist() -> HashMap<String, bool> {
    ...
    let yaml_arr = YamlLoader::load_from_str(
        &read_to_string(&filename).expect("Could not load blocklist file"),
    )
